...
def compute_expression_signature_scores(gene_df, signature_defs, min_genes=2):
    ...
    return pd.DataFrame(score_frames, index=gene_df.index), pd.DataFrame(manifest_rows)


def attach_biology_signatures(df):
    df = df.copy()
    existing = [name for name in BIO_SIGNATURES if name in df.columns]
    if existing:
        return df, sorted(existing), pd.DataFrame()

    gene_like = [
        c for c in df.columns
        if c not in CLINICAL
        and not str(c).startswith("gene_programme_")
        and c not in BIO_SIGNATURES
    ]
    if not gene_like:
        return df, [], pd.DataFrame()

    gene_df = df[gene_like].apply(pd.to_numeric, errors="coerce").clip(lower=-10, upper=10)
    gene_df = gene_df.fillna(gene_df.median(axis=0)).fillna(0.0)
    scores, manifest = compute_expression_signature_scores(gene_df, BIO_SIGNATURES)
    for col in scores.columns:
        df[col] = scores[col].astype(float)
    return df, scores.columns.tolist(), manifest
...
# ══════════════════════════════════════════════════════════════════════════════
# FIG 5a — Pathway / microenvironment scores by PAM50
# ══════════════════════════════════════════════════════════════════════════════
def fig_signature_pam50_heatmap(df_train, signature_cols, out):
    selected = [
        "pathway_luminal_score",
        "pathway_proliferation_score",
        "pathway_her2_score",
    ...
    selected = [c for c in selected if c in signature_cols and c in df_train.columns]
    if not selected:
        return

    label_map = {
        "pathway_luminal_score": "Luminal",
        "pathway_proliferation_score": "Proliferation",
        "pathway_her2_score": "HER2",
        "pathway_immune_ifn_score": "Immune/IFN",
        "pathway_emt_stroma_score": "EMT/Stroma",
        "stromal_matrix_score": "Stromal matrix",
        "immune_innate_score": "Innate/IFN",
    }
    ...
    for sig in selected:
        ...
        groups = []
        for sub in PAM50_ORDER:
            vals = df_train.loc[df_train["pam50_+_claudin-low_subtype"] == sub, sig].dropna()
        ...
        if len(groups) >= 2:
            ...
            f, p = np.nan, np.nan
        f_rows.append((label_map.get(sig, sig), f, p))

    heat = pd.DataFrame(mean_mat, index=[label_map.get(s, s) for s in selected], columns=PAM50_ORDER)
    heat = heat.sub(heat.mean(axis=1), axis=0)
    std = heat.std(axis=1).replace(0, np.nan)
    heat = heat.div(std, axis=0).fillna(0.0)
    ...
    sns.heatmap(
        ...
        fmt=".2f",
        linewidths=0.5,
        cbar_kws={"shrink": 0.7, "label": "Row-standardised mean score"},
        ax=axes[0],
    )
    axes[0].set_title(
        "Pathway and microenvironment scores by PAM50 subtype\n"
        "Means computed on training data and row-standardised for comparison"
    )
    axes[0].tick_params(axis="x", rotation=35, labelsize=8)
    axes[0].tick_params(axis="y", rotation=0, labelsize=9)

    f_df = pd.DataFrame(f_rows, columns=["Signature", "F", "p"]).sort_values("F", ascending=True, na_position="first")
    colors = ["#e74c3c" if (pd.notna(f) and f > 40) else "#f39c12" if (pd.notna(f) and f > 10) else "#3498db" for f in f_df["F"]]
    bars = axes[1].barh(f_df["Signature"], f_df["F"].fillna(0), color=colors, alpha=0.85)
    for bar, (_, row) in zip(bars, f_df.iterrows()):
        p = row["p"]
        p_txt = "p<0.001" if pd.notna(p) and p < 0.001 else (f"p={p:.3f}" if pd.notna(p) else "p=NA")
        axes[1].text(float(bar.get_width()) + 0.5, bar.get_y() + bar.get_height()/2,
                     f"F={row['F']:.1f}  {p_txt}" if pd.notna(row["F"]) else "F=NA  p=NA",
                     va="center", fontsize=8)
    axes[1].set_xlabel("ANOVA F-statistic (score by PAM50 subtype)")
    axes[1].set_title(
        "Discriminative power of pathway / TME scores\n"
        "High F indicates that the score separates known biological subtypes"
    )

    fig.suptitle(
        "Section 3 — Pathway and Microenvironment Validation\n"
        "Expression-derived signature scores recover expected subtype biology",
        fontsize=12, fontweight="bold"
    )
    fig.tight_layout()
    save(fig, out / "20_pathway_microenvironment_heatmap.png")


# ══════════════════════════════════════════════════════════════════════════════
# FIG 5b — Concordance between NMF programmes and biology signatures
# ══════════════════════════════════════════════════════════════════════════════
def fig_signature_gp_concordance(df_train, gp_cols, signature_cols, out):
    spotlight_gps = [gp for gp in ["gene_programme_01", "gene_programme_05", "gene_programme_10", "gene_programme_14"] if gp in gp_cols]
    selected = [
        "pathway_luminal_score",
        "pathway_proliferation_score",
        "pathway_her2_score",
    ...
    selected = [c for c in selected if c in signature_cols and c in df_train.columns]
    if not spotlight_gps or not selected:
        return

    label_map = {
        "gene_programme_01": "GP01 luminal",
        "gene_programme_05": "GP05 proliferation",
        "gene_programme_10": "GP10 HER2/epithelial",
        "gene_programme_14": "GP14 immune/IFN",
        "pathway_luminal_score": "Luminal",
    ...
    corr = pd.DataFrame(index=[label_map[g] for g in spotlight_gps], columns=[label_map[s] for s in selected], dtype=float)
    pvals = {}
    for gp in spotlight_gps:
        for sig in selected:
            sub = df_train[[gp, sig]].dropna()
            if len(sub) >= 3:
                r, p = stats.pearsonr(sub[gp], sub[sig])
            else:
                r, p = np.nan, np.nan
            corr.loc[label_map[gp], label_map[sig]] = r
            pvals[(label_map[gp], label_map[sig])] = p
    ...
    axes[0].set_title(
        "Concordance between NMF gene programmes and formal biology scores\n"
        "Correlations computed on the training subset"
    )
    axes[0].tick_params(axis="x", rotation=35, labelsize=8)
    axes[0].tick_params(axis="y", rotation=0, labelsize=9)

    flat = corr.stack().reset_index()
    flat.columns = ["Programme", "Signature", "r"]
    flat["abs_r"] = flat["r"].abs()
    flat = flat.sort_values("abs_r", ascending=False).head(10)

    axes[1].axis("off")
    axes[1].text(0.5, 0.97, "Top concordant pairs", ha="center", va="top", fontsize=11, fontweight="bold", color=HEAD_BG)
    y = 0.90
    for _, row in flat.iterrows():
        p = pvals.get((row["Programme"], row["Signature"]), np.nan)
        p_txt = "p<0.001" if pd.notna(p) and p < 0.001 else (f"p={p:.3f}" if pd.notna(p) else "p=NA")
        axes[1].text(
            0.03, y,
            f"{row['Programme']} ↔ {row['Signature']}: r={row['r']:.2f} ({p_txt})",
            transform=axes[1].transAxes, va="top", fontsize=8.6, color=CELL_FG,
        )
        y -= 0.075
    axes[1].text(
        0.03, max(0.08, y - 0.03),
        "Interpretation: strong alignment between GP01/05/10/14 and formal luminal, proliferation, HER2 and immune scores\n"
        "supports that the NMF layer is capturing known breast-tumour biology rather than arbitrary latent structure.",
        transform=axes[1].transAxes, va="top", fontsize=8.8, color=HEAD_BG,
    )

    fig.suptitle(
        "Section 4 — Programme-to-Pathway Concordance\n"
        "Formal signature scores corroborate the manual biological labels assigned to the key NMF programmes",
        fontsize=12, fontweight="bold"
    )
    fig.tight_layout()
    save(fig, out / "21_programme_signature_concordance.png")
...
def main():
    p = argparse.ArgumentParser(description="Notebook 05 — Clinical and molecular validation (biology enhanced)")
    ...
    p.add_argument("--outdir", type=Path, default=Path("outputs") / "notebook_05")
    args = p.parse_args()
    ...
    df.loc[df["er_status_measured_by_ihc"].isin(["Posyte", "Positve"]), "er_status_measured_by_ihc"] = "Positive"
    df.loc[df["cancer_type_detailed"] == "Breast", "cancer_type_detailed"] = np.nan
    df.loc[df["geo_location_id"] == 0, "geo_location_id"] = np.nan
    df.loc[~df["age_at_diagnosis"].between(18, 100), "age_at_diagnosis"] = np.nan
    if df["death_from_cancer"].dtype == object:
        df["death_from_cancer"] = df["death_from_cancer"].map({"Died of Disease": 1, "Died of Other Causes": 0, "Living": 0})
    ...
    df, signature_cols, _ = attach_biology_signatures(df)

    h_path = args.outputs_dir / "METABRIC_nmf_components.csv"
    nmf_model_path = args.outputs_dir / "nmf" / "nmf_model.joblib"
    nmf_scaler_path = args.outputs_dir / "nmf" / "nmf_minmax_scaler.joblib"
    train_idx_path = args.outputs_dir / "metadata" / "shared_train_indices.csv"

    if not h_path.exists():
        raise FileNotFoundError(f"NMF components not found: {h_path}")
    H = pd.read_csv(h_path, index_col=0)

    gp_cols = [c for c in df.columns if str(c).startswith("gene_programme_")]
    if not gp_cols:
        if not (nmf_model_path.exists() and nmf_scaler_path.exists()):
            raise FileNotFoundError("NMF model/scaler artefacts not found; run the enhanced pipeline first.")
        gene_like = [c for c in raw.columns if c not in CLINICAL]
        gene_df = df[gene_like].apply(pd.to_numeric, errors="coerce").clip(lower=-10, upper=10)
        gene_df = gene_df.fillna(gene_df.median(axis=0)).fillna(0.0)
        kept_genes = [c for c in H.columns if c in gene_df.columns]
        scaler = joblib.load(nmf_scaler_path)
        nmf = joblib.load(nmf_model_path)
        full_scaled = np.clip(scaler.transform(gene_df[kept_genes]), 0.0, 1.0)
        W_full = nmf.transform(full_scaled)
        gp_cols = [f"gene_programme_{i+1:02d}" for i in range(W_full.shape[1])]
    ...
    fig_km_pam50(df, args.outdir)
    fig_km_grade_receptor(df, args.outdir)
    fig_gp_pam50_heatmap(df_train, gp_cols, args.outdir)
    fig_gp_spotlight(df_train, gp_cols, H, args.outdir)
    if signature_cols:
        fig_signature_pam50_heatmap(df_train, signature_cols, args.outdir)
        fig_signature_gp_concordance(df_train, gp_cols, signature_cols, args.outdir)
    fig_clinical_correlations(df, args.outdir)
    fig_cohort_confounding_mitigation(df_train, gp_cols, args.outdir)
    fig_research_questions(df, gp_cols, df_train, args.outdir)

    print(f"\nSaved Notebook 05 figures to {args.outdir}")
