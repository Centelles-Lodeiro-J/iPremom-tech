...
"""
Feature Selection — final runnable version
METABRIC Breast Cancer Dataset

Main chain support script.

Methods
-------
M1, M3, M4:
  1. Random Forest importances
  2. LASSO / multinomial / ordinal proxy coefficients
  3. Mutual Information

M2 (lifelines-only):
  1. Univariate Cox PH (p-value ranking on training set)
  2. Penalised multivariable Cox absolute coefficients
  3. Cox drop-column C-index importance on training set

A feature is selected if it appears in at least min_votes methods.
All selection is fitted on training data only.
"""
...
PAL = {"rf":"#3498db","lasso":"#e74c3c","mi":"#2ecc71","cox":"#9b59b6","consensus":"#2c3e50"}
...
def parse_args():
    p=argparse.ArgumentParser(description='Feature selection — final runnable version')
...
def plot_model(score_series, title, out):
    ...
    if len(score_series)==1: axes=[axes]
...
def run_model(model_key, label, splits_dir, top_n, min_votes):
    ...
    outdir=splits_dir/model_key/'feature_selection'; outdir.mkdir(parents=True, exist_ok=True)
    task=MODELS[model_key]['task']
    if task=='binary':
        y=y_tr_raw.iloc[:,0].astype(int)
        rf_imp, rf_sel = run_rf(X_tr, y, top_n)
        lasso_coef, lasso_sel, alpha = run_lasso(X_tr, y)
        mi_scores, mi_sel = run_mi(X_tr, y, top_n)
        rf_imp.to_csv(outdir/'rf_importances.csv', header=['importance'])
        lasso_coef.to_csv(outdir/'lasso_coefficients.csv', header=['coef_abs'])
        mi_scores.to_csv(outdir/'mutual_information.csv', header=['mi'])
        consensus_df, selected = build_consensus({'rf':rf_sel,'lasso':lasso_sel,'mi':mi_sel}, X_tr.columns, min_votes)
        plot_model({'rf':rf_imp,'lasso':lasso_coef,'mi':mi_scores}, label, outdir/'plot.png')
    elif task=='multi':
        y=y_tr_raw.iloc[:,0]
        rf_imp, rf_sel = run_rf(X_tr, y, top_n)
...
def main():
    ...
    results = {}
    print('=' * 65)
    print('FEATURE SELECTION — final runnable version')
    print('=' * 65)
    for mk in MODELS:
        if not (args.splits_dir / mk).exists():
            continue
        print(f"\n── {mk} ──")
        results[mk] = run_model(mk, mk.replace('_', ' '), args.splits_dir, args.top_n, args.min_votes)
        print(f"    Selected: {results[mk]['n_selected']}")
    rows = [{'model': k, 'n_selected': v['n_selected']} for k, v in results.items()]
    pd.DataFrame(rows).to_csv(args.output_dir / 'feature_selection_summary.csv', index=False)
    print('\nDone.')
